    ...
    <a-card title="权限系统测试页面" style="margin: 20px;">
      ...
      <a-card type="inner" title="用户信息" style="margin-bottom: 16px;">
        <a-descriptions :column="2">
          <a-descriptions-item label="用户名">{{ userStore.user?.username || '未登录' }}</a-descriptions-item>
      ...
      <a-card type="inner" title="权限控制测试" style="margin-bottom: 16px;">
        <a-row :gutter="16">
          ...
          <a-col :span="12">
            ...
            <!-- 复合权限测试 -->
            <div v-auth="{ permissions: ['user:read', 'user:write'], mode: 'any' }" style="margin-bottom: 8px;">
              <a-tag color="cyan">v-auth (any user permissions) - 显示</a-tag>
            </div>
            
            <div v-auth="{ permissions: ['user:read', 'user:write'], mode: 'all' }" style="margin-bottom: 8px;">
          ...
          <a-col :span="12">
            <a-form layout="vertical">
              <a-form-item label="检查权限">
                ...
                />
                <a-button type="primary" @click="checkPermission" style="margin-top: 8px;">
                  检查权限
                </a-button>
              </a-form-item>
            </a-form>
          ...
          <a-col :span="12">
            <a-form layout="vertical">
              <a-form-item label="检查角色">
                ...
                />
                <a-button type="primary" @click="checkRole" style="margin-top: 8px;">
                  检查角色
                </a-button>
              </a-form-item>
            </a-form>
            ...
            <template #renderItem="{ item }">
              <a-list-item>
                <a-tag :color="item.result ? 'green' : 'red'">
                  {{ item.type }}: {{ item.value }} - {{ item.result ? '有权限' : '无权限' }}
                </a-tag>
              </a-list-item>
            </template>
          </a-list>
      ...
      <a-card type="inner" title="当前用户权限">
        <a-row :gutter="16">
          <a-col :span="12">
            ...
            <div v-if="!permissionStore.userPermissions.length" style="color: #999;">
              暂无权限
            </div>
          </a-col>
