...
export const usePermissionStore = defineStore('permission', () => {
  ...
  const isAdmin = computed(() => {
    return userRoles.value.some(
      role => role.name === ROLES.SUPER_ADMIN || role.name === ROLES.ADMIN
    )
  })
  ...
  const isManager = computed(() => {
    return userRoles.value.some(
      role =>
        role.name === ROLES.SUPER_ADMIN || role.name === ROLES.ADMIN || role.name === ROLES.MANAGER
    )
  })
  ...
  const hasPermission = permissionCode => {
    ...
    return userPermissions.value.some(
      permission => permission.code === permissionCode && permission.is_active
    )
  }
  ...
  const hasRole = roleName => {
    ...
    return userRoles.value.some(role => role.name === roleName && role.is_active)
  }
  ...
  const getRoleNames = computed(() => {
    return userRoles.value.filter(role => role.is_active).map(role => role.name)
  })
